...
/** Render the player HUD boxes (name on top, score below) from PlayersHudData.players. */
function renderScoreboard(hudPlayers) {
  ...
  scoreboardList.innerHTML = ordered
    .map((p) => {
      ...
      return (
        `<div class="player-box" style="border-left-color: ${color}">` +
...
/**
 * Render overhead nametags from HudScreenPointData.points. Mirrors the platform:
 * x 0..1 left→right and y 0..1 bottom→top, so the CSS top uses (1 - y).
 */
function renderScreenPoints(points) {
  screenPoints.innerHTML = points
    .filter((pt) => pt.type === "playerOverhead" && !pt.isOffScreen)
    .map((pt) => {
      const name = nameById.get(pt.playerId) ?? `#${pt.playerId}`;
      const left = pt.x * 100;
      const top = (1 - pt.y) * 100;
      return `<div class="overhead" style="left: ${left}%; top: ${top}%">${name}</div>`;
    })
    .join("");
}
